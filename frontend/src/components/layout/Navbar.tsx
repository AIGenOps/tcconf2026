...
export default function Navbar() {
  ...
  return (
    <>
      ...
      >
        <div
          className={`mx-auto flex items-center justify-between px-6 py-3 rounded-full transition-all duration-500 max-w-5xl border bg-transparent border-transparent ${
            scrolled
              ? "xl:glass-panel xl:shadow-[0_10px_30px_-10px_rgba(0,82,255,0.15)] xl:border-white/10"
              : "xl:bg-[#0a0a0c]/85 xl:border-white/5 xl:backdrop-blur-md"
          }`}
        >
          {/* Logo (Unified inside the bar, left-aligned) */}
          <Link href="/" className="hidden xl:flex items-center group mr-4 flex-shrink-0">
            <svg className="h-7 w-auto text-thunder-cyan transition-transform duration-300 group-hover:scale-105 drop-shadow-[0_0_8px_rgba(0,240,255,0.3)]" viewBox="0 0 873 657" fill="currentColor">
      ...
      >
        ...
        >
          ...
          <span className="max-w-0 overflow-hidden group-hover:max-w-xs group-hover:ml-3 text-xs font-bold tracking-widest text-slate-100 uppercase transition-all duration-500 whitespace-nowrap font-mono">
            Buy Ticket
          </span>
        </Link>
      </motion.div>
